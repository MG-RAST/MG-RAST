package AnnotationClearingHouse::WebPage::conflict_single;
...
#    my $fig = $self->application->data_handle('FIG');
    ...
    my $exp1 = $cgi->param('exp1') || "";
    my $exp2 = $cgi->param('exp2') || "";
    my $func1 = $cgi->param('funcA') || "";
    my $func2 = $cgi->param('funcB') || "";

    # get data
    my $diffs = $dbf->SQL("SELECT t1.id,t1.function,t1.expert,t2.id,t2.function,t2.expert FROM 
                       ACH_Assertion as t1, ACH_Assertion as t2, ACH_Correspondence WHERE
                       ACH_Correspondence.status = 'd' AND 
                       t1.function =\"$func1\" AND 
                       t2.function =\"$func2\" AND
                     ...
                     ");
    $self->app->add_message('info' , "You have " . scalar @$diffs . " conflicts based on different IDs for this function pair: <ul><li>$func1<li>$func2</ul>" );
    my $tab = &make_table($diffs,$cgi,$fig);
    my $table = $self->application->component('Conflicts');
    $table->data($tab);
    $table->columns( [ { 'name' => 'ID1' },
    ...
    push(@$html,$cgi->h2("ACH Resolution - ID List")); 
    push(@$html,"<p>Dear $expert, this is a list of ID pairs. Each ID in a pair refers to the same sequence. The functions attached to this IDs are $func1 <b>and</b> $func2:</p>");
...
sub make_table {
    ...
    return [map { 
		  ...
		  else
		  {
		      ...
		      my $idL2 = &prot_link($cgi,$expert,$id2);
...
sub count_link{
  ...
  return "<a href='?page=conflicts_single&exp1=$exp1&&exp2=$exp2&funcA=$func1&funcB=$func2'>".$count."</a>";
}
