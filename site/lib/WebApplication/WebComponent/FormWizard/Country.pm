package WebComponent::FormWizard::Country;
...
sub unencode_value {
  ...
  return exists($countries->{$value}) ? $countries->{$value} : $value;
}
...
sub output {
    ...
    foreach my $c (sort {$countries->{$a} cmp $countries->{$b}} keys %$countries) {
      my $select = ($default eq $c) ? " selected='selected'" : "";
      $content  .= "<option value='$c'$select>" . $countries->{$c} . "</option>";
    }
    $content .= "</select>";
