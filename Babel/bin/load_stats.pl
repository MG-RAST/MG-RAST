...
print "Adding source md5 counts to db ...\n" if ($verbose);
my $t_md5 = $dbh->selectcol_arrayref(qq(SELECT COUNT (*) FROM (
SELECT DISTINCT md5 FROM md5_protein UNION
SELECT DISTINCT md5 FROM md5_ontology UNION
SELECT DISTINCT md5 FROM md5_rna ) AS x));
&add_count( "md5s", $t_md5->[0] );
