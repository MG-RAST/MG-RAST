...
foreach my $input (@files){
	...
	print STDERR "No input file $indir/$input\n";
	next;
    }
...
#   print $entry->database_code,"\n";
	...
	#print "\n\n";
	if ( $entry->DRs->get("GO") ){
	    my @GOs = $entry->DRs->get("GO");
	    #print Dumper $entry->DRs->get("GO");
	    
	    my $source = 'GOs from Uniprot';
	    foreach my $go (@GOs){
		
		print SEQ ">".$go->[1]."\n"; 
		print SEQ $entry->SQ, "\n";
		print FUNC $go->[1]."\t". $go->[2], "\n";
		print ORG  $go->[1]. $entry->OSs->head->text, "\n";
		print MD5 "$md5\t".$go->[1]."\t".$go->[2]."\t".$entry->OSs->head->text."\t$source\n";
