...
sub new {
    ...
    # check 
    if (! $dbh) {
      $dbh = DBI->connect("DBI:$Global_Config::babel_dbtype:dbname=$Global_Config::babel_db;host=$Global_Config::babel_dbhost", $Global_Config::babel_dbuser, '');
...
sub md5s2idfunc4sources {
  ...
  foreach my $t (keys %types) {
    if ($t ne 'ontology') {
      push @sqls, "select distinct md5, source, id, function, organism from md5_$t where md5 in ($md5_list) and source in ($src_list)";
    }
    else {
  ...
  if ($rows && (@$rows > 0)) {
	...
	if ( $r->[3] && exists($src_map{$r->[1]}) && exists($func_map{$r->[3]}) ) {
	  my $oid = $r->[4] ? $r->[4] : 0;
	  push @{ $data->{$r->[0]} }, [ $src_map{$r->[1]}, $r->[2], $func_map{$r->[3]}, $oid ];
	}
      }
    }
  }
...
sub get_organism_id_from_name {
  my ($self, $orgs) = @_;

  unless ($orgs && (@$orgs > 0)) { return {}; }
  my $data = {};
  my $rows = $self->dbh->selectall_arrayref("select name, _id from organisms_ncbi where name in (".join(",", map {$self->dbh->quote($_)} @$orgs).")");
  if ($rows && (@$rows > 0)) {
