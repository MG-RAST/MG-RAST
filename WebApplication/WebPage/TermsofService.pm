...
sub output {
  ...
  my $version = $Global_Config::require_terms_of_service;
...
sub check_terms {
  ...
  if ($user && $master && $cgi->param('iagree')) {
    ...
    if (scalar(@$pref) && $pref->[0]->value >= $Global_Config::require_terms_of_service) {
      $application->add_message('info', "You had already agreed to the terms of service.");
    } else {
      if (scalar(@$pref)) {
	$pref->[0]->value($Global_Config::require_terms_of_service);
      } else {
	$pref = $master->Preferences->create( { user => $user,
						name => 'AgreeTermsOfService',
						value => $Global_Config::require_terms_of_service } );
	if ($pref) {
	  if ($cgi->param('nih')) {
