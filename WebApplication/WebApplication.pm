...
sub new {
  ...
  my $self = { cgi         => $cgi,
	       ...
	       bot_no_follow => 0,
	       strict_browser => 0,
	       anonymous_mode => $Global_Config::anonymous_mode || 0,
               transmitted  => 0,
	       require_terms_of_service => $Global_Config::require_terms_of_service || 0,
	       in_request => 0,
	       js_init_functions => [],
	     };
...
sub register_component {
  ...
  if (scalar(@{$self->{'components'}->{$component}}) && $self->{'components'}->{$component}->[0]->{singleton}) {
    ...
    unless  (ref $web_component) {
      $self->error("Error: Failed on new, unable to init component '$component'.");
    }
  ...
  if ($self->{'component_index'}->{$id}) {
    $self->error("Error: Duplicate component id '$id'.");
  }
...
sub add_message {
  ...
  if($fadetimer && $fade_flag){
    ...
    $msg .= "<img src=\"$Global_Config::cgi_url/Html/clear.gif\" onload='fade(\"" . $div_id . "\", ". $fadetimer . ");'>";
  }
...
sub check_for_maintenance {
  ...
  if (-f $Global_Config::global."/why_down") {
    open(MSG, $Global_Config::global."/why_down")
...
sub check_requirements {
  ...
  if ($user_agent && $user_agent =~ /MSIE/) {
    $self->layout->add_css_reverse("$Global_Config::cgi_url/Html/ie7-css.css");
    $js->{"$Global_Config::cgi_url/Html/ie7-standard-p.js"} = 1;
  }

  # include message fade script
  $js->{"$Global_Config::cgi_url/Html/fade.js"} = 1;
...
sub check_browser_strict {
  ...
  if ($user_agent =~ /Firefox\/(\d+)/) {
    ...
    return (0, "This application has been optimized for the Firefox browser. Since you are using $ua, some features may not be available. Firefox is freely available <a href='http://www.mozilla.org/firefox/'>here</a>.");
  }
}
...
sub get_user_info {
  ...
  if ($session->user) {
    if ($self->fancy_login) {
      $info .= "<div id='user'>";
      $info .= "<div id='userdisplay' style='float:left;'>".$session->user->firstname . " " . $session->user->lastname."</div><div style='float:left;'>";
	...
	$info .= "<div id='login_submit'><input type='submit' value='login' style='margin-right: 0; width:45px;cursor: pointer;' title='Click here to login!'></div></div>" .$self->page->end_form();
      } else {
	$info .= $self->page->start_form('login_form', { page => 'Login', action => 'perform_login' });
...
sub run {
  ...
  my $package = $self->backend->name().'::WebPage::'.$page;
  {
    ...
    if ($@) {
      ...
      my $possible_error = $@;
      my $last_error = $!;
	  $package = $self->{'parent'}.'::WebPage::'.$page;	  
      eval "require $package";
      if ($@) {
