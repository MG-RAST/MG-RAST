...
____END
    ...
    $layout->set_relocation("$Global_Config::cgi_url/");
    ...
    my $retVal = WebApplication::new($class,
                                      { id       => $applicationID,
                                        cgi      => $cgi,
                                        noTrace  => 1,
                                        dbmaster => DBMaster->new(-database => $Global_Config::webapplication_db,
    ...
    $retVal->url("$Global_Config::cgi_url/wiki/rest.cgi/NmpdrPlugin/$applicationID");
    # Return the object.
    return $retVal;
}
