...
sub handle {
  ...
  unless (exists $_[0]->{_handle}) {
    if (! $Global_Config::rast_jobcache_db) {
      # Denote no RAST without putting a confession in the error log.
      $_[0]->{_handle} = undef;
    } else {
      eval {
	$_[0]->{_handle} = DBMaster->new( -database => $Global_Config::rast_jobcache_db || 'JobCacheRast',
					  -host     => $Global_Config::rast_jobcache_host,
					  -user     => $Global_Config::rast_jobcache_user,
					  -password => $Global_Config::rast_jobcache_password );
      };
      if ($@) {
