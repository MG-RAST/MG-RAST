...
In most cases, the Global_Config will determine whether your get an SFXlate object
or a FIG object. If C<$Global_Config::nmpdr_mode> is TRUE, you get SFXlate. If it's
FALSE, you get FIG.
...
sub handle {
  ...
  unless(exists($self->{_fig})) {
    ...
    if ($mode eq 'FIG') {
      ...
      $self->{_fig} = SFXlate->new(undef, $Global_Config::oldSproutDB, undef, $Global_Config::oldSproutDBD);
    } else {
  ...
  if ($Global_Config::rast_jobs && $self->application->session->user) {
	...
	push(@fig_m_dirs, map { $Global_Config::rast_jobs . "/" . $_ } @$fig_m_orgs);
  ...
  if ($Global_Config::mgrast_jobs) {

    # no rast on anno3, even if config variable exists
    unless ($Global_Config::anno3_mode) {
