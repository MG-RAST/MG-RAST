...
sub output {
  ...
  $image .= "<map name=\"circplot_map_" . $self->id . "\">".join("\n", @$map)."</map>".qq~<img id="plot_img_~ . $self->id . qq~" style="border: none;" usemap="#circplot_map_~ . $self->id . qq~" src="~ . $self->image->image_src() ."\"><img src=\"$Global_Config::cgi_url/Html/dot.gif\" id='plot_dot_".$self->id."' onload='navigate_plot(\"".$self->id."\", \"1\", \"1\");' style='position: relative;'>";
...
sub require_javascript {
  return ["$Global_Config::cgi_url/Html/CircularPlot.js"];
}
