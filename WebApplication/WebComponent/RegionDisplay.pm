...
sub require_javascript {
    ...
    return ["$Global_Config::cgi_url/Html/RegionDisplay.js"];
}
...
sub pinned_regions_form {
	...
	($check1, $check2) = ($self->collapse_close_genomes() == 1)? ('checked', '') : ('', 'checked');
	$form .= "<tr><th>Genome selection</th><td>" .
	         "<input type='radio' name='collapse_close_genomes' value='1' $check1> Collapse close genomes " .
		 "<input type='radio' name='collapse_close_genomes' value='0' $check2> Show all</td></tr>";
    ...
    if ($Global_Config::anno3_mode || (ref($fig) eq 'FIGV') || ((ref($fig) eq 'FIGM') && exists($fig->{_figv_cache}->{$org}))) {
	# print STDERR ref($self->application->session->user) . " " . $self->application->session->user->has_right(undef, 'annotate', 'genome', $org) . "\n";
      if ($self->application->session->user && $self->application->session->user->has_right(undef, 'annotate', 'genome', $org)) {
