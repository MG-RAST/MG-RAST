...
function fill_table (id, start, stop, show_prev, show_next) {
  ...
  for (var rownum=start; rownum<stop; rownum++) {
	...
	if (table_onclick[data_index] && table_onclick_data[data_index][relative_rownum]) {
	...
	/* check for cell highlighting */
	var bgcolor = "";
	if (table_highlight_data[data_index] && table_highlight_data[data_index][relative_rownum][colnum]) {
	...
	var click = "";
	if (table_onclick_data[data_index] && table_onclick_data[data_index][relative_rownum][colnum]) {
...
function sort_numeric_up (a, b) {
  aa = parseFloat(a[SORT_COLUMN_INDEX].replace(HTML_REPLACE, "").replace(/,/, ""));
  if (isNaN(aa)) return 1;
  bb = parseFloat(b[SORT_COLUMN_INDEX].replace(HTML_REPLACE, "").replace(/,/, ""));
