...
sub output {
  ...
  if ($self->small_login) {
    if ($self->application->session->user) {
      ...
      $content .="&nbsp;<img src=\"$Global_Config::cgi_url/Html/login.png\" onclick='document.getElementById(\"login_form\").submit();' title='Login' style='width: 24px; height: 24px; cursor: pointer;'>";
...
sub perform_login {
  ...
  if (ref $user and crypt($password, $user->password) eq $user->password) {
    ...
    if ($user->active and ($user->has_right($self->application, 'login') || $Global_Config::open_gates)) {
...
sub require_css {
  return "$Global_Config::cgi_url/Html/Login.css";
}
