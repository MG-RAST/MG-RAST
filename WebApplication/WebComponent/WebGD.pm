...
sub image_src {
  ...
  if ($user_agent eq 'IE' || $Global_Config::file_images_only) {
    my ($fh, $filename) = tempfile( TEMPLATE => 'webimageXXXXX', DIR => $Global_Config::temp, SUFFIX => '.png' );
    print $fh $self->png();
    close $fh;
    $filename =~ s/.*\/(\w+\.png)$/$1/;
    $image_link = $Global_Config::temp_url."/".$filename;
  } else {
