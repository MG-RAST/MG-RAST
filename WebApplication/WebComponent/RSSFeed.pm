...
sub output_editor {
  ...
  opendir(my $dh, $Global_Config::temp) || die "can't open feed directory: $!";
  my @feeds = grep { /\.rss$/ && -f "$Global_Config::temp/$_" } readdir($dh);
  closedir $dh;
...
sub file_path {
  ...
  my $path = $Global_Config::temp . "/" . $self->file_name . ".rss";
...
sub url {
  ...
  my $url = $Global_Config::temp_url . "/" . $self->file_name . ".rss";
...
sub require_javascript {
  return ["$Global_Config::cgi_url/Html/datepickercontrol.js"];
}

sub require_css {
  return "$Global_Config::cgi_url/Html/RSSFeed.css";
}
