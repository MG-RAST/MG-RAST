...
sub output {
  ...
  $content .= "<p width=800px align=justify >You can also find the answers to frequently asked questions on our <a href='http://blog.mg-rast.org'>support page</a>.</p>";
...
sub try_contact {
  ...
  if ( $result->{is_valid} ) {
    ...
    my $email_success = MGRAST::Mailer::send_email(   from => $cgi->param('email'),
                                                      to => "help\@mg-rast.org",
