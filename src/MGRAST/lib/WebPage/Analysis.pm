...
sub recruitment_plot_graph {
  ...
  my $config_file    = "$Config::temp/circos_$unique_str.conf";
  my $karyotype_file = "$Config::temp/karyotype_$unique_str.txt";
  my $fwd_gene_file  = "$Config::temp/genes_fwd_$unique_str.txt";
  my $rev_gene_file  = "$Config::temp/genes_rev_$unique_str.txt";
  my $evals_file     = "$Config::temp/evals_$unique_str.txt";
  ...
  if ((-s $config_file) && (-s $karyotype_file) && (-s $fwd_gene_file) && (-s $rev_gene_file) && (-s $evals_file)) {
    ...
    if (-s "$Config::temp/$circos_file.png") {
...
<<include $Config::mgrast_config_dir/ideogram.conf>>
<<include $Config::mgrast_config_dir/ticks.conf>>
...
sub workbench_blat_output {
  ...
  my ($fgs_infile, $fgs_infile_name) = tempfile( "fgs_in_XXXXXXX", DIR => $Config::temp, SUFFIX => '.faa');
  print $fgs_infile $fgs_infile_content;
  close $fgs_infile;
  my ($nr_file, $nr_file_name) = tempfile( "nr_XXXXXXX", DIR => $Config::temp, SUFFIX => '.faa');
  print $nr_file $nr_seq_data;
  close $nr_file;
  my $fgs_cmd = $Config::fraggenescan_executable." -s $fgs_infile_name -o " . $fgs_infile_name . ".fgs -w 0 -t 454_30";
  `$fgs_cmd`;
...
sub get_read_align {
  ...
  if ((@md5_seq == 2) && ($md5_seq[0] =~ /$md5/)) {
    my $md5_fasta = $Config::temp."/".$md5."_".time.".faa";
    ...
    foreach my $s (@$seq_data) {
      ...
      my $read_fasta  = $Config::temp."/".$mgid."_".$id."_".time.".fna";
...
sub qiime_export_data {
  ...
  my $data = { "id"                  => "",
	       ...
	       "type"                => $tbl_type,
	       "generated_by"        => "MG-RAST revision ".$Config::server_version,
...
sub phylogeny_visual {
  ...
  if ($cgi->param('vis_type') eq 'vbar') {
    ...
    if (defined($cgi->param('pval'))) {
      ...
      my ($pvalgroupf, $pvalgroupn) = tempfile( "rpvalgXXXXXXX", DIR => $Config::temp, SUFFIX => '.txt');
      print $pvalgroupf join("\t", map { $mg2group->{$_} } @comp_mgs)."\n";
      close $pvalgroupf;
      my ($pvaldataf, $pvaldatan) = tempfile( "rpvaldXXXXXXX", DIR => $Config::temp, SUFFIX => '.txt');
	...
	$i++;
      ...
      close $pvaldataf;
      my ($pvalsuggestf, $pvalsuggestn) = tempfile( "rpvalsXXXXXXX", DIR => $Config::temp, SUFFIX => '.txt');
      close $pvalsuggestf;
      my ($pvalresultf, $pvalresultn) = tempfile( "rpvalrXXXXXXX", DIR => $Config::temp, SUFFIX => '.txt');
      close $pvalresultf;
      my ($pvalexecf, $pvalexecn) = tempfile( "rpvaleXXXXXXX", DIR => $Config::temp, SUFFIX => '.txt');
      my $rn = "normalized";
      if ($cgi->param('raw')) {
	$rn = "raw";
      }
      print $pvalexecf "source(\"".$Config::r_scripts."/suggest_stat_test.r\")\n";
	...
	$settings .= "<tr><td>$cmg</td><td>".$mg2group->{$cmg}."</td></tr>";
      ...
      $settings .= "</table><br>";
      my ($pvalexec2f, $pvalexec2n) = tempfile( "rpvale2XXXXXXX", DIR => $Config::temp, SUFFIX => '.txt');
      print $pvalexec2f "source(\"".$Config::r_scripts."/do_stats.r\")\n";
...
#    my $transposer = "<input type='button' value='transpose' onclick='transpose_table(\"".$t->id."\", 0, [2,3,4], 10,\"transpose_space_".$tabnum."\");'><div id='transpose_space_".$tabnum."'></div>";
  ...
  if ($cgi->param('vis_type') eq 'heatmap' || $cgi->param('vis_type') eq 'pca') {
	...
	push(@$heatmap_data, $row);
      ...
      my ($fh, $infile) = tempfile( "rdataXXXXXXX", DIR => $Config::temp, SUFFIX => '.txt');
	...
	print $fh join("\t", @$row)."\n";
      ...
      my ($prefh, $prefn) =  tempfile( "rpreprocessXXXXXXX", DIR => $Config::temp, SUFFIX => '.txt');
      print $prefh "source(\"".$Config::r_scripts."/preprocessing.r\")\n";
      print $prefh "MGRAST_preprocessing(file_in = \"".$infile."\", file_out = \"".$Config::temp."/rdata.preprocessed.$time\", image_out =\"".$Config::temp."/$boxfile\", produce_fig = \"TRUE\")\n";
      close $prefh;
	...
	unlink $infile;
	$infile = $Config::temp."/rdata.preprocessed.$time";
      }
	...
	my ($col_f, $row_f) = ($Config::temp."/rdata.col.$time", $Config::temp."/rdata.row.$time");

	my ($heath, $heatn) =  tempfile( "rheatXXXXXXX", DIR => $Config::temp, SUFFIX => '.txt');
	print $heath "source(\"".$Config::r_scripts."/dendrogram.r\")\n";
	...
	$content .= $self->heatmap_scale($max_val)."<div id='heatmap_canvas_$tabnum'></div><img src='".$Config::temp_url."/$boxfile' width=600>";
	...
	my $time = time;
	my ($pca_data) = ($Config::temp."/rdata.pca.$time");
	my ($pcah, $pcan) =  tempfile( "rpcaXXXXXXX", DIR => $Config::temp, SUFFIX => '.txt');
	print $pcah "source(\"".$Config::r_scripts."/plot_pco.r\")\n";
	...
	$content .= $img_control;
	$content .= "<table><tr><td><div id='pca_canvas_$tabnum'></div></td><td>$data_download_button".$comp_control.$group_control."</td></tr></table><img src='".$Config::temp_url."/$boxfile' width=600>";
...
sub recruitment_plot_visual {
  ...
  if ($cgi->param('vis_type') eq 'circle') {
    ...
    $content .= qq~<div><div>Recruitment Plot Map $tabnum</div><div>~.clear_progress_image().qq~<p><span style='font-size: 1.2em'><b>Hits for $mgid mapped on $name</b></span></p><p><a href="$Config::temp_url/$file.png">download image here</a></p><table><tr><th>Hits Distribution by e-Value Exponent Range</th><th>Summary of Mapped Hits</th></tr><tr><td><img src="$eval_hist"/></td><td><table><tr><td>Features Mapped</td><td>$stats->[0]</td></tr><tr><td>Features Covered</td><td>$stats->[1]</td></tr><tr><td>Total Features</td><td>$stats->[2]</td></tr><tr><td>Contigs Shown</td><td>$plotctg</td></tr><tr><td>Total Contigs</td><td>$allctg</td></tr></table></td></tr></table><div><p><img width="960" src="$Config::temp_url/$file.png" onmouseover="TJPzoom(this);" /></p></div></div></div>~;
    $tabnum++;
  }
...
sub qiime_export_visual {
  ...
  unless (-f $Config::temp."/download.$fn") {
    ...
    if (ref($data)) {
      ...
      my $json = new JSON;
      $json = $json->utf8();
      if (open(FH, ">".$Config::temp."/download.$fn")) {
...
sub download {
  ...
  my $cgi = $self->application->cgi;
  my $file = $Config::temp."/download.".$cgi->param('file');
...
##################
sub require_css {
  return [ "$Config::cgi_url/Html/Analysis.css" ];
}

sub require_javascript {
  return [ "$Config::cgi_url/Html/Analysis.js", "$Config::cgi_url/Html/heatmap.js", "$Config::cgi_url/Html/pca.js", "$Config::cgi_url/Html/canvg.js", "$Config::cgi_url/Html/rgbcolor.js", "$Config::cgi_url/Html/rarefaction.js", "$Config::cgi_url/Html/zoom.js", "$Config::cgi_url/Html/krona.js" ];
}
