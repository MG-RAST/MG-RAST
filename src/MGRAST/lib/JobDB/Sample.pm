...
sub create {
   ...
   unless ($user) {
     die "sample creation requires a user";
   }
   ...
   if (exists($values->{id})) {
     die "Sample-id is auto generated";
   }

   if (exists($values->{libraries})) {
     die "Sample-libraries are set automatically";
   }

   if (exists($values->{metagenome})) {
     die "Sample-metagenome is set automatically";
   }
   ...
   my $id = ($self->_master->MetaDataCollection->last_id) + 1;
   my $collection = { ID => $id,
		      name => $values->{name},
		      entry_date => $values->{creation},
		      creator => $user,
		      type => 'sample' };
   my $coll = $self->_master->MetaDataCollection->create($collection);

   unless (ref($coll)) {
     die "unable to create sample: $@";
   }

   $self->_master->Rights->create( { scope     => $user->get_user_scope, 
				     data_type => 'sample', 
				     data_id   => $id , 
				     name      => 'view', 
				     granted   => 1 ,
   ...
   if (exists($values->{metadata}) && ref($values->{metadata}) eq 'HASH') {
     foreach my $key (keys(%{$values->{metadata}})) {
       $self->_master->MetaDataEntry->create( { value => $values->{metadata}->{$key},
						tag => $key,
						collection => $coll } );
     }
   }
   ...
   $obj->{about} = "sample";
   $obj->{name} = $coll->{name};
   $obj->{url} = $url_base.'/sample/'.$obj->{id};
   $obj->{version} = 1;
   $obj->{creation} = $coll->{entry_date};
   ...
   # get libraries
   my $libraries = $self->_master->MetaDataCollection->get_objects( { ID => [ $coll->{ID}.'.%', 'like' ] } );
   my $lids = [];
   @$lids = map { "mgl".$_->{ID} } @$libraries;
   $obj->{libraries} = $lids;
   
   # create a new object from result hash
   my $object = $package->_new_from_hash($self->_master, $obj);
...
sub delete {
  die "sample objects cannot be deleted";
}
...
sub get_data {
  ...
  my $params = { type => 'template', 'ID' => [ '%.%', 'not like' ] };
  if ($values && $values->{id}) {
    $values->{id} =~ s/^mgs(.*)/$1/;
    $params->{ID} = $values->{id};
  }
