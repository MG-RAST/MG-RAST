    ...
    my $content = { 'name' => $self->name,
		            ...
		            'requests' => [
				        ...
				        { 'name'        => "normalize",
				          ...
				          'example'     => [ 'curl -X POST -H "auth: auth_key" -d \'{'.$self->{example}.'}\' "'.$self->cgi->url."/".$self->name.'/normalize"',
						...
						{ 'name'        => "heatmap",
				          ...
				          'example'     => [ 'curl -X POST -H "auth: auth_key" -d \'{"raw":0,"cluster":"mcquitty"'.$self->{example}.'}\' "'.$self->cgi->url."/".$self->name.'/heatmap"',
				          ...
				          'parameters'  => { 'options'  => {},
							                 'required' => {},
							                 'body'     => { "auth" => ["string", "unique string of text generated by MG-RAST for your account"],
     							                             "data" => ['list', ['list', 'raw or normalized value']],
           							                         "rows" => ['list', ['string', 'row id']],
           							                         "columns" => ['list', ['string', 'column id']],
						...
						{ 'name'        => "pcoa",
				          ...
				          'description' => "Calculate a PCoA for given input data.",
				          'example'     => [ 'curl -X POST -H "auth: auth_key" -d \'{"raw":1,"distance":"euclidean"'.$self->{example}.'}\' "'.$self->cgi->url."/".$self->name.'/pcoa"',
