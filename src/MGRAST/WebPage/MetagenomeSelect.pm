...
#  my $data_description = [ 'job_id', 'jobname', 'metagenome_id', 'project', 'biome', 'altitude', 'depth', 'location', 'ph', 'country', 'temperature', 'sequencing method', 'PI' ];
  ...
  foreach my $row (@$data_table){
    ...
    if (exists $row->{'biome'} and $row->{'biome'} ne '' and $row->{'biome'} ne ' ' and $row->{'biome'} ne '0' and $row->{'biome'} ne ' - ' and $row->{'biome'} ne "unknown") {
      $genome_id2biome->{$row->{'metagenome_id'}} = $row->{'biome'};
  ...
  $cdtable->columns( [ 'metagenome', 'project', 'type', 'biome', 'remove' ] );
  ...
  foreach my $k (keys(%$cdata_hash)) {
    ...
    foreach my $v (@{$cdata_hash->{$k}}) {
      ...
      my $cds = $name_link."~~".$project_link."~~".$project_type."~~".($genome_id2biome->{$data->{$v}{jobname}[0]} || "-")."~~<input type='button' value='remove' onclick='remove_single(&quot;$k^$v&quot;);'>~~".$ind;
      $cds =~ s/"/\@1/g;
      $cds =~ s/'/\@2/g;
      push(@$cddata, $cds);
      $ind++;
...
sub change_collection {
  ...
  foreach my $k (keys(%$cdata_hash)) {
    ...
    foreach my $v (@{$cdata_hash->{$k}}) {
      ...
      my $project_link = $genome_id2project->{$data->{$v}{jobname}[0]} ? "<a href='?page=MetagenomeProject&project=$pid' target=_blank>".$genome_id2project->{$data->{$v}{jobname}[0]}."</a>" : "-";
      my $project_type = $genome_id2project->{$data->{$v}{jobname}[0]} ? $project_hash->{$genome_id2project->{$data->{$v}{jobname}[0]}}->{type} : "-";
      my $cds = $name_link."~~".$project_link."~~".$project_type."~~".($genome_id2biome->{$data->{$v}{jobname}[0]} || "-")."~~<input type='checkbox'>~~".$ind;
      $cds =~ s/"/\@1/g;
      $cds =~ s/'/\@2/g;
      push(@$cddata, $cds);
      $ind++;
