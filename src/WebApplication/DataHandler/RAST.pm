...
sub handle {
  ...
  unless (exists $_[0]->{_handle}) {
    if (! $Config::rast_jobcache_db) {
      # Denote no RAST without putting a confession in the error log.
      $_[0]->{_handle} = undef;
    } else {
      eval {
	$_[0]->{_handle} = DBMaster->new( -database => $Config::rast_jobcache_db || 'JobCacheRast',
					  -host     => $Config::rast_jobcache_host,
					  -user     => $Config::rast_jobcache_user,
					  -password => $Config::rast_jobcache_password );
      };
      if ($@) {
