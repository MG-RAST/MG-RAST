package WebComponent::ModelTable;

# ModelTable - a modified version of the table component designed to enhance performance of tables showing data from the model database

# $Id: ModelTable.pm,v 1.2 2009-12-21 19:59:26 chenry Exp $
...
sub output {
  ...
  if ($self->{urlparameters}) {
    $table .= "<input type='hidden' id='table_urlarguments" . $self->id() . "' value='" . $self->{urlparameters} . "'>\n";
  }
  $table .= "<input type='hidden' id='table_column_names" . $self->id() . "' value='" . $ColumnNames . "'>\n";
  ...
  $table .= "<input type='hidden' id='table_start_" . $self->id() . "' value='0'>\n";
  ...
  if (ref($self->show_export_button())) {
    ...
    $export_button .= "<input type='button' value='$title' onclick='export_table(\"" . $self->id() . "\", $unfiltered, $strip_html, $hide_invisible_columns);'>";
  } elsif ($self->show_export_button()) {
    $export_button .= "<input type='button' value='export table' onclick='export_table(\"" . $self->id() . "\");'>";
  }
  
  # check for clear filter button
  my $clear_filter_button = "";
  if ($self->show_clear_filter_button()) {
    $clear_filter_button = "<input type='button' value='clear all filters' onclick='table_reset_filters(\"" . $self->id() . "\");'>";
  }

  my $buttons = "";
  if ($export_button || $clear_filter_button) {
    $buttons = "<tr><td>".$export_button.$clear_filter_button."</td></tr>";
  }
  ...
  if ($self->show_select_items_per_page) {
    ...
    $select_items_per_page .= "<tr><td style='width: 100%; text-align: center; vertical-align: middle;'><table style='width: 100%;'>\n<tr><td align=center><span class='table_perpage'>display&nbsp;<input type='text' id='table_perpage_" . $self->id() . "' name='table_perpage_" . $self->id() . "' size='3' value='" . $self->items_per_page() . "' onkeypress='return check_submit_filter(event, \"" . $self->id() . "\");'>&nbsp;items per page</span></td></tr></table></td></tr>\n";
     ...
     $select_items_per_page .= "<input type='hidden' id='table_perpage_" . $self->id() . "' name='table_perpage_" . $self->id() . "' value='" . scalar(@{$self->data}) . "' >\n";
  } else {
  ...
  if ($self->show_bottom_browse) {
    $bottombrowse = "<tr><td style='width: 100%; text-align: center;'>".$self->get_browse()."</td></tr>";  
  }
  ...
  foreach my $col (@{$self->columns}) {
	...
	$filter .= "</select><input type='text' name='" . $name . "' class='filter_item' value='" . $operand . "' id='table_" . $self->id() . "_operand_" . $i . "' onkeypress='return check_submit_filter(event, \"" . $self->id() . "\");' style='width: 70%;'>";
      } elsif (defined($col->{operator}) && ($col->{operator} eq 'combobox')) {
	push(@$combo_columns, $i);
	my %coldata = map { $_->[$i - 1] => 1 } @{$self->data()};
	$filter = "<br><input type=hidden name='" . $name . "_operator' value='equal' id='table_" . $self->id() . "_operator_" . $i . "'><select name='" . $name . "' class='filter_item' id='table_" . $self->id() . "_operand_" . $i . "' onchange='return check_submit_filter2(\"" . $self->id() . "\");' style='width: 100%;' title='Select Filter'>";
 	$filter .= "</select>";
      } else {
	my $operator = 'like';
	...
	$filter = "<br><input type=hidden name='" . $name . "_operator' value='" . $operator . "' id='table_" . $self->id() . "_operator_" . $i . "'><input type='text' name='" . $name . "' class='filter_item' value='" . $operand . "' size=5 id='table_" . $self->id() . "_operand_" . $i . "' onkeypress='return check_submit_filter(event, \"" . $self->id() . "\");' style='width: 100%;' title='Enter Search Text'>";
      }
    }
    ...
    if ($col->{sortable}) {
      $col->{name} = "<a href='javascript: table_sort(\"" . $self->id() . "\", \"" . $i . "\", \"ASC\");' class='table_first_row' title='Click to sort'>" . $col->{name} . "&nbsp;<img src='./Html/up-arrow.gif'><img src='./Html/down-arrow.gif'></a>";
    }
  ...
  $table .= "<input type='hidden' id='table_visible_columns_" . $self->id() . "' value='" . join('@~', @{$self->{visible_columns}}) . "'>\n";
  ...
  $table = "<table class='table_table'>".$buttons.$select_items_per_page.$topbrowse.$control_panel."<tr><td>".$table."</td></tr>".$bottombrowse."</table>";
...
sub get_browse {
  ...
  my $to = scalar(@{$self->data});

  my $browse .= "<table style='width: 100%;'><tr><td align='left' width='20%'>" . $left . "</td><td align='center' width='60%'>displaying <span name='table_start_" . $self->id() . "'>" . ($self->offset() + 1) . "</span> - <span name='table_stop_" . $self->id() . "'>" . $to . "</span> of <span name='table_total_" . $self->id() . "'>" . scalar(@{$self->data}) . "</span></td><td align='right' width='20%'>" . $right . "</td></tr></table>";
...
sub submit_button {
  ...
  my $html = "<input type='button' value='$button_name' onclick='table_submit(\"" . $self->id() . "\", \"$form_name\", \"$submit_all\");'>";
...
sub format_new_column_data {
  ...
  # check if colum header click should sort
  if ($col->{sortable}) {
    $col->{name} = "<a href='javascript: table_sort(\"" . $self->id() . "\", \"" . $col->{num} . "\", \"ASC\");' class='table_first_row' title='Click to sort'>" . $col->{name} . "&nbsp;<img src='./Html/up-arrow.gif'><img src='./Html/down-arrow.gif'></a>";
  }
  ...
  $new_column = qq~<input type='hidden' id='table_~.$self->id().qq~_new_column_data' value='~.$new_column."**".$new_data.qq~'><img src='./Html/clear.gif' onload="var a=document.getElementById('table_~.$self->id().qq~_new_column_data').value.split('**');table_append_data('~.$self->id().qq~', a[0], a[1]);">~;
...
sub format_data {
  ...
  my $data = $self;
  my $id = 0;
  if (ref($self) eq "WebComponent::ModelTable") {
    $data = $self->data();
...
sub require_javascript {
  return ['./Html/ModelTable.js', './Html/PopupTooltip.js'];
}
