...
sub output {
  ...
  # set the image path
  my $img_path = "$Config::cgi_url/Html/";
...
sub format_new_column_data {
  ...
  $new_column = qq~<input type='hidden' id='table_~.$self->id().qq~_new_column_data' value='~.$new_column."**".$new_data.qq~'><img src=\"$Config::cgi_url/Html/clear.gif\" onload="var a=document.getElementById('table_~.$self->id().qq~_new_column_data').value.split('**');table_append_data('~.$self->id().qq~', a[0], a[1]);">~;
...
sub require_javascript {
  return ["$Config::cgi_url/Html/Table.js", "$Config::cgi_url/Html/PopupTooltip.js"];
}

sub require_css {
  return "$Config::cgi_url/Html/Table.css";
}
