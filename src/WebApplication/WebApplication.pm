...
sub new {
  ...
  my $self = { cgi         => $cgi,
	       ...
	       bot_no_follow => 0,
	       strict_browser => 0,
	       anonymous_mode => $Config::anonymous_mode || 0,
               transmitted  => 0,
	       require_terms_of_service => $Config::require_terms_of_service || 0,
	       in_request => 0,
	       js_init_functions => [],
	     };
...
sub add_message {
  ...
  if($fadetimer && $fade_flag){
    ...
    $msg .= "<img src=\"$Config::cgi_url/Html/clear.gif\" onload='fade(\"" . $div_id . "\", ". $fadetimer . ");'>";
  }
...
sub check_requirements {
  ...
  if ($user_agent && $user_agent =~ /MSIE/) {
    $self->layout->add_css_reverse("$Config::cgi_url/Html/ie7-css.css");
    $js->{"$Config::cgi_url/Html/ie7-standard-p.js"} = 1;
  }

  # include message fade script
  $js->{"$Config::cgi_url/Html/fade.js"} = 1;
