package MGRAST::WebPage::PCA_ssav;
...
sub output {
  ...
  if($cgi->param('job_ids')){
	  ...
	  my $dir = $jobs_string;
	  $dir =~s/,//g;
   	  my $link =  $Global_Config::temp_url."/$dir/pc1_vs_pc2.pdf";
	  $html .= "<a href='$link'>PC1 vs PC2 </a>";
	  $html .= "<br><br>";
	  my $pc_table = $self->application->component('Principal_Components');
	  my $columns = [{ 'name' => 'Subsystem', 'filter' => 1}, { 'name' => 'PC1', 'sortable'=> 1} , { 'name' => 'PC2','sortable' => 1}];
	  ...
	  my $rotational_matrix_file = $Global_Config::temp."/$dir/pca_rotational_matrix.txt";
...
sub run_pca_utility{
    ...
    my $dir = $jobs_string;
    $dir =~s/,//g;
    my $output_dir = $Global_Config::temp."/$dir";
    if(! -d $output_dir){ `mkdir $output_dir`;}
    
    $jobs_string =~s/,/ /g;
    `compute_PCA_for_metagenomes $output_dir $jobs_string`;
