...
sub run_blast {
    ...
    my $query_file = "$Global_Config::temp/tmp.$$.fasta";
    $self->print_fasta($seq_id, $seq, $query_file);

    # assemble blastall command
    my $cmd  = "$Global_Config::ext_bin/blastall";
...
sub run_formatdb_if_needed {
    ...
    {
	my $cmd = "$Global_Config::ext_bin/formatdb -i $db_path/$db_file -n $db_path/$db_file -p F -t '$metagenome_name' -l $db_path/formatdb.log";
	my $fig = $self->fig();
	$fig->run($cmd);
    }
}
