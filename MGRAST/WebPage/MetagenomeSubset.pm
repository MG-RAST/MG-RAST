...
sub align_sequences {
  ...
  if (scalar @selected_seqs > 0){
    my $job_id = time();
    my $temp_file = "$Global_Config::temp/$job_id.fasta";
    ...
    $ENV{HOME_4_TCOFFEE} = "$Global_Config::temp/";
    $ENV{DIR_4_TCOFFEE} = "$Global_Config::temp/.t_coffee/";
    $ENV{CACHE_4_TCOFFEE} = "$Global_Config::temp/cache/";
    $ENV{TMP_4_TCOFFEE} = "$Global_Config::temp/tmp/";
    $ENV{METHOS_4_TCOFFEE} = "$Global_Config::temp/methods/";
    $ENV{MCOFFEE_4_TCOFFEE} = "$Global_Config::temp/mcoffee/";

    my @cmd = ("$Global_Config::ext_bin/t_coffee","$temp_file", "-output", "score_html", "-outfile", "$Global_Config::temp/$job_id.html", "-run_name", "$Global_Config::temp/$job_id","-quiet","$Global_Config::temp/junk.txt");

    my $command_string = join(" ",@cmd);
    open(RUN,"$command_string |");
    while($_ = <RUN>){}
    close(RUN);
    open(HTML,"$Global_Config::temp/$job_id.html");
    while($_ = <HTML>){
      $_ =~s/<html>//;
      $_ =~s/<\/html>//;
      $content .= $_;
...
sub require_javascript{
    return ["$Global_Config::cgi_url/Html/checkboxes.js"];
}
