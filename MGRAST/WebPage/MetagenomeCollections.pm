...
sub output {
  ...
  if ($self->{use_buffer} && -f $Global_Config::temp."/mgs_temp_data") {
    if (open(FH, $Global_Config::temp."/mgs_temp_data")) {
      $self->{data} = <FH>;
      close FH;
    } else {
      $self->fetch_data();
...
sub require_javascript {
  return [ "$Global_Config::cgi_url/Html/MetagenomeCollections.js" ];
}
