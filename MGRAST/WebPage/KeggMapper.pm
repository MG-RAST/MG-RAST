...
sub output {
  ...
  open(FH, $Global_Config::mgrast_data . "/kegg/keggdata.poly") or die "oh noes! $@ $!";
  my $polys = <FH>;
  chomp $polys;
  close FH;

  my $names = [];
  open(FH, $Global_Config::mgrast_data . "/kegg/keggdata.names") or die "oh noes! $@ $!";
  while (<FH>) {
    chomp;
    push(@$names, $_);
  }
...
sub require_javascript {
  return ["$Global_Config::cgi_url/Html/Kegg.js", "$Global_Config::cgi_url/Html/raphael-min.js", "$Global_Config::cgi_url/Html/canvg.js"];
}
...
sub highlight_select {
  ...
  my $select = "<h2>Data Selection</h2><form name='meta_form' id='meta_form' onkeypress='return event.keyCode!=13'><input type='hidden' name='metagenome' value='".$mg."'><table id='non_wb_sel'><tr><td style='font-weight: bold; width: 200px;'>Target Buffer</td><td><select id='tbuff' name='tbuff'><option value='buffer_space_a'>Data A</option><option value='buffer_space_b'>Data B</option></select></td></tr><tr><td style='font-weight: bold; width: 200px;'>Transcriptomes</td><td id='mg_sel_td'>".$metagenome."</td><td>".$self->more_button('document.getElementById("sel_mg").style.display="";', 'ok_button("'.$mg_sel->id.'");')."</td></tr><tr><td colspan=3 style='display: none;' id='sel_mg'><table><tr><td>".$mg_sel->output()."</td><td><input type='button' value='ok' onclick='ok_button(\"".$mg_sel->id."\");'></td></tr></table></td></tr></table><input type='button' value='load data' onclick='if(document.getElementById(\"list_select_list_b_".$self->application->component('ls')->id."\").options.length){list_select_select_all(\"".$self->application->component('ls')->id."\");execute_ajax(\"get_kegg_data\",\"kdatabuf\",\"meta_form\",\"loading...\", null, tobuff);}else{alert(\"You did not select any metagenomes\");};'><input type='hidden' name='source' value='KO'></form>";

#<tr><td style='font-weight: bold;'>Max. e-Value Cutoff</td><td>".$self->get_evals->[0]."</td><td>".$self->more_button('document.getElementById("meta_sel_eval").style.display="";')."</td><td style='display: none;' id='meta_sel_eval'>".$self->evalue_select()."</td></tr><tr><td style='font-weight: bold;'>Min. % Identity Cutoff</td><td>".$self->get_idents->[0]."</td><td>".$self->more_button('document.getElementById("meta_sel_ident").style.display="";')."</td><td style='display: none;' id='meta_sel_ident'>".$self->identity_select()."</td></tr><tr><td style='font-weight: bold;'>Min. Alignment Length Cutoff</td><td>".$self->get_alens->[0]."</td><td>".$self->more_button('document.getElementById("phylo_sel_alen").style.display="";')."</td><td style='display: none;' id='phylo_sel_alen'>".$self->alength_select()."</td></tr>
...
#  my $al_index = $self->get_alen_index($alength);
  ...
  $result = $self->{mgdb}->get_ontology_for_source($source, undef, undef, undef);#$ev_index, $id_index, $al_index);
