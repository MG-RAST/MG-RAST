...
sub init {
  ...
  my $job;
  eval { $job = $mgrast->Job->init({ metagenome_id => $id }); };
  unless ($job) {
  ...
  foreach my $mgid (@$mgids) {
    ...
    my $j = $mgrast->Job->init( { metagenome_id => $mgid } );
    next unless $j;
...
sub share_job {
  ...
  unless ($email =~ /^[\w\-\.]+\@[\.a-zA-Z\-0-9]+\.[a-zA-Z]+$/) {
	...
	@$genome_ids = map { $_->{value} } @{$self->data('mgids')};
      }
    ...
    $self->application->add_message('warning', 'Please enter a valid email address. If you tried to share with a ');
    return 0;
  }
  ...
  if (ref $user) {
    ...
    if ($cgi->param('multishare') && $cgi->param('multishare') == 1) {
      ...
      $what = "the metagenomes\n".join("\n", map { $mghash->{$_}." (".$_.")" } @$mgids);
      $genome_id = $mgids->[0]->{value};
    }
