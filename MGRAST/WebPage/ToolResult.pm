...
sub output {
	...
	my $peg2 = $cgi->param('peg2');
        my $job_id1 = time() - 1;
        my $temp_file1 = "$Global_Config::temp/$job_id1.seq1.fasta";
	my $temp_file2 = "$Global_Config::temp/$job_id1.seq2.fasta";
	my $temp_out = "$Global_Config::temp/$job_id1.out";
        open(OUT,">$temp_file1");
	...
	system("$Global_Config::ext_bin/bl2seq -p blastp -i $temp_file1 -j $temp_file2 > $temp_out");
	open (FH, $temp_out);
	while (my $line = <FH>){
	    if ($line =~ /Expect =/){
	...
	system("$Global_Config::ext_bin/bl2seq -p blastx -i $temp_file1 -j $temp_file2 > $temp_out");
	open (FH, $temp_out);
	while (my $line = <FH>){
	    if ($line =~ /Expect =/){
		my ($garbage) = $line =~ /(Expect .*)/;
