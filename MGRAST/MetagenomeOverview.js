...
function draw_rank_abundance_plot(data_id, display_id) {
    // Grab the data
    ...
    draw_generic_plot(data, labels, plabel, display_id, 'hit', 'Logrithmic Scale', true, true);
}
...
function draw_histogram_plot(data_id, display_id, unit, ltext) {
    ...
    draw_generic_plot(data, labels, plabel, display_id, 'sequence', ltext, false);
}
...
function draw_generic_curve(data, display_id, xlabel, ylabel, xtext, ytext) {
    ...
    // Raphael.fn.drawGrid = function (x, y, w, h, wv, hv, color)
    r.drawGrid(leftgutter + X * .5 + .5, topgutter + .5, maxX - X, maxY, 10, 10, "#333");
...
function check_download(obj) {
  if (confirm('Download all reads hitting '+obj+' as FASTA')) {
