...
sub new {
  ...
  my $self = { app      => $app,
	       debug    => $debug,
	       template => $Global_Config::mgrast_formWizard_templates . "/FormWizard_MetaData.xml"
	     };
  eval {
      $self->{_handle} = DBMaster->new( -database => $Global_Config::mgrast_metadata_db || 'MGRASTMetadata',
					-host     => $Global_Config::mgrast_metadata_host,
					-user     => $Global_Config::mgrast_metadata_user,
...
sub export_metadata_for_jobs {
  ...
  my $fpath = "$Global_Config::temp/$file";
  my $keys  = {};
...
sub get_projects {
  ...
  if ( $opt->{ privat } ) {
    ...
    $results = $dbh->selectcol_arrayref("SELECT DISTINCT id FROM Project where public=1 and type='study'");
  }
